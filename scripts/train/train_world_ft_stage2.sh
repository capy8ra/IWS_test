#!/usr/bin/env bash
# Stage 2 (latent dynamics + right-arm joint-torque head) on the world_ft data.
# Predicts future image latents + 8 joint torques from image history + q_des.
# Encoder/decoder are loaded frozen from the Stage-1 checkpoint (load_ae).
...
# the most recent Stage-2 checkpoint of this experiment.
RESUME_ARG=()
LATEST_CKPT="$(ls -t outputs/world_ft_stage_2/*/checkpoints/last.ckpt 2>/dev/null | head -n1 || true)"
if [[ -n "${LATEST_CKPT}" ]]; then
  echo "[train] Resuming from latest checkpoint: ${LATEST_CKPT}"
  RESUME_ARG=("load=${LATEST_CKPT}")
else
  echo "[train] No previous Stage-2 checkpoint found -- starting from scratch."
fi
