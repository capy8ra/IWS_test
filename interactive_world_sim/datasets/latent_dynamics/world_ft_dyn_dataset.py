...
Adds, on top of the base dataset:
  ...
  * ``normalizer["joint_torque"]``       -> per-joint Gaussian standardization
...
class WorldFtDynDataset(RealAlohaDataset):
    """RealAlohaDataset + per-frame right-arm joint-torque target."""
    ...
    def get_normalizer(self, mode: str = "none", **kwargs) -> LinearNormalizer:
        ...
        # per-joint Gaussian standardization, fit on the (train) replay buffer
        normalizer["joint_torque"] = SingleFieldLinearNormalizer.create_fit(
            self.replay_buffer["joint_torque"], last_n_dims=1, mode="gaussian"
        )
        return normalizer
